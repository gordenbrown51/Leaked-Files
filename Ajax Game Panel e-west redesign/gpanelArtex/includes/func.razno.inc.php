...
{
	if(preg_match('/^[A-Z][a-zA-Z - šđčćžŠĐČĆŽ]+$/', $ime))
	{
		return TRUE;
	}
	else
...
function avatar($id) 
{
	...
	{
		if($margin) $avatar = "<img src='/avatar.php?mode=c&id={$id}'/>";
		else $avatar = "<img src='/avatar.php?mode=c&id={$id}'/>";
		return $avatar;
		exit;
	}
}
...
function tikett_status_mirza_panel($status)
{
	global $jezik;	
	if($status=="1")
	{
		$st = '<p class="bg-green c-white bold panel-header-status">'.$jezik['text275'].'</p>';
		return $st;
	}
	else if($status=="4")
	{
		$st = '<p class="bg-yellow c-white bold panel-header-status">'.$jezik['text276'].'</p>';
		return $st;
	}
	else if($status=="2")
	{
		$st = '<p class="bg-yellow c-white bold panel-header-status">'.$jezik['text278'].'</p>';
		return $st;
	}
	else if($status=="3")
	{
		$st = '<p class="bg-red c-white bold panel-header-status">'.$jezik['text279'].'</p>';
		return $st;
	}
	else if($status=="5")
	{
	...
	{
		$st = '<p class="bg-yellow c-white bold panel-header-status">'.$jezik['text277'].'</p>';
		return $st;
	}
	return FALSE;
}
...
function igra_no_name($i)
{
	if($i == "1") $igra = "<img width='16' height='16' src='./assets/img/game-cs.png' />";
	else if($i == "2") $igra = "<img width='16' height='16' src='./assets/img/game-samp.png' />";
	else if($i == "3") $igra = "<img width='16' height='16' src='./assets/img/game-minecraft.png' />";
	else if($i == "4") $igra = "<img width='16' height='16' src='./assets/img/game-cs.png' />";
	else if($i == "5") $igra = "<img width='16' height='16' src='./assets/img/game-cs.png' />";
	return $igra;
}
...
function srv_istekao($srvid)
{
	...
	if($s > $d)
	{
		...
		$istekao = "<span class='c-blue'>".$datum."</span> (".$jezik['text284']." <span class='c-blue'>".$dana[1]."</span> ".$jezik['text285'].")";
		$istekao .= "<form action='process.php' method='POST'>
						...
						<button style='padding:0;background:transparent;font-size:10px;'class='c-yellow' type='submit'>[ ".$jezik['text286']." ]</button>
					</form>";
	} 
	else
	{
		...
		if($dana == "0")
		{
			...
			$istekao .= "<form action='process.php' method='POST'>
							...
							<input type='hidden' name='srvid' value='".$srvid."'>
							<input type='hidden' name='klijentid' value='".$_SESSION['klijentid']."'>
							<button style='padding:0;background:transparent;font-size:10px; class='c-yellow bg-white' type='submit'>[ ".$jezik['text286']." ]</button>
						</form>";		
		}
		else
		{
			$istekao = "<span class='c-blue'>".$datum."</span> (".$jezik['text15123']." ".$jezik['text602']." <span class='c-blue'>".$dana."</span> ".$jezik['text285'].")";
		}
	}
	return $istekao;
}
...
function resize($cover = false, $format2){
