...
if(isset($_GET['mode']))
{
	if($_GET['mode'] == "klijenti")
	{
		...
		$query = "SELECT * FROM klijenti WHERE email LIKE '%" . $s . "%' OR concat(' ',ime,' ',prezime,' ') LIKE '%" . $s . "%' LIMIT 5";
		...
		if((mysql_num_rows($squery) != 0) && ($s != ""))
		{
			while($sLookup = mysql_fetch_array($squery))
			{
				...
				$output .= '<li style="overflow: hidden;" style="cursor: pointer;" onclick="sendToSearch(\'' . $displayName . '\')"><div style="float:left;">' . avatar($sLookup['klijentid'], 30, 30) . '</div><div style="margin-left: 45px; display: block; float: left; margin-left: 3px;"> ' . $ime . ' ' . $prezime . ' 
